...
# Check events
kubectl get events -n dclaw-vendor --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-vendor deployment/dclaw-vendor-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-vendor-db-credentials -n dclaw-vendor
   ```
