...
### How do I update DClaw Vendor?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp vendor --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Vendor without Kubernetes?

Yes, for local development:

```bash
cd dclaw-vendor/frontend && npm run dev
cd dclaw-vendor/backend && uvicorn dclaw_vendor.main:app --reload
```
